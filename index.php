...
<?php else: ?>
  ...
  <script>
    ...
    async function filesFromDropEvent(e){const transfer=e.dataTransfer;const fallbackFiles=[...(transfer.files||[])];const items=[...(transfer.items||[])];if(!items.length)return fallbackFiles;const dropped=items.map(item=>{const entry=item.webkitGetAsEntry?item.webkitGetAsEntry():null;const file=!entry&&item.getAsFile?item.getAsFile():null;return{entry,file}});const hasDirectory=dropped.some(item=>item.entry&&item.entry.isDirectory);if(!hasDirectory&&fallbackFiles.length)return fallbackFiles;const batches=await Promise.all(dropped.map(item=>item.entry?readEntry(item.entry,""):item.file?[item.file]:[]));const files=batches.flat();return files.length?files:fallbackFiles}
