...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .file-table th:nth-child(2),.file-table td:nth-child(2){width:32%}
    .file-table th:nth-child(3),.file-table td:nth-child(3){width:36%}
...
<?php else: ?>
  ...
  <script>
    ...
    function truncateNameCompact(name,isDir=false){
      ...
      const max=w>=680?28:w>=520?24:w>=430?22:w>=380?20:17;
      return truncateName(name,isDir?Math.max(14,max-4):max);
    }
    ...
    function renderRow(item){
      ...
      const displayPath=truncatePath(item.path,compactMobile?18:(mediumScreen?22:28));
    ...
    async function uploadFiles(files){files=[...files];if(!files.length)return;const form=new FormData();form.append("path",state.path);for(const file of files){form.append("files[]",file,file.name);form.append("paths[]",fileRelativePath(file))}uploadProgress.classList.add("show");uploadProgress.textContent=`Uploading ${files.length} file(s)...`;try{const r=await fetch(apiUrl("upload",{path:state.path}),{method:"POST",credentials:"same-origin",body:form});const j=await r.json();const uploadedCount=(j.uploaded||[]).length;if(uploadedCount)toast(j.message||`${uploadedCount} file(s) uploaded.`);if(j.errors&&j.errors.length)toast(summarizeUploadErrors(j.errors));if(!uploadedCount&&(!j.errors||!j.errors.length)&&!j.ok)throw new Error(j.error||"Upload failed");await loadFolder()}catch(e){toast(e.message||String(e))}finally{uploadProgress.classList.remove("show");uploadInput.value="";uploadFolderInput.value=""}}
